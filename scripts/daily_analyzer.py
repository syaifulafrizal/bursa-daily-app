...
# BURSA INTEL UNIVERSE - 120+ TOP SHARIAH STOCKS
ASSET_METADATA = {
    # UTILITIES & RE
    ...
    "0215.KL": {"name": "SLVEST", "sector": "Utilities"}, "5014.KL": {"name": "YTLPOWR", "sector": "Utilities"},
    ...
    "1155.KL": {"name": "MAYBANK", "sector": "Other"}, "1295.KL": {"name": "PBBANK", "sector": "Other"},
    ...
    "5222.KL": {"name": "DAYANG", "sector": "Energy"}, "5141.KL": {"name": "DAX", "sector": "Energy"},
    "5027.KL": {"name": "MSNIAGA", "sector": "Energy"}, "5218.KL": {"name": "SAPNRG", "sector": "Energy"},
    "5245.KL": {"name": "DELEUM", "sector": "Energy"}, "5132.KL": {"name": "COASTAL", "sector": "Energy"},
    ...
    "5250.KL": {"name": "7ELEVEN", "sector": "Consumer"}, "6947.KL": {"name": "DIGI", "sector": "Other"},
    "6012.KL": {"name": "MAXIS", "sector": "Other"}, "5095.KL": {"name": "HEINEKEN", "sector": "Other"},
    # HEALTHCARE
    ...
    "5148.KL": {"name": "UEMS", "sector": "Property"}, "5249.KL": {"name": "IOIPROP", "sector": "Property"},
    "5279.KL": {"name": "MATRIX", "sector": "Property"}, "1503.KL": {"name": "OSK", "sector": "Property"},
    # INDUSTRIAL
...
def get_top_movers():
    print(f"Scanning Bursa Universe ({len(ASSET_METADATA)} assets)...")
    base_tickers = list(ASSET_METADATA.keys())
    active_data = []
    for ticker_symbol in base_tickers:
        try:
            ...
            meta = ASSET_METADATA[ticker_symbol]
            # Simple Shariah Filter (Removing banks/gaming manually if they slipped into pool)
            if meta["sector"] == "Other": continue

            active_data.append({
                "ticker": ticker_symbol, "name": meta["name"], "sector": meta["sector"],
                "price": round(price, 3), "change_pct": round(change, 2),
                "vol_spike": round(volume / avg_vol, 2) if avg_vol > 0 else 1.0
            })
            time.sleep(0.02) # Faster scanning for larger universe
        except: continue
    
    # MOMENTUM SCORING: Find the real movers across the whole 120-stock pool
...
def rule_based_analysis(market_data):
    report = {
        ...
        "market_overview": "Broad market scanning active. Current rotation is being detected in mid-cap Shariah counters across Infrastructure and Consumer sectors.",
        "top_picks": []
    }
    for i, m in enumerate(market_data[:12]):
        ...
        report["top_picks"].append({
            ...
            "analysis": f"Detected significant momentum in {m['name']} ({m['sector']}). Price shifted {m['change_pct']}% on {m['vol_spike']}x normal volume. Technical structure remains favorable."
        })
    return report
...
def analyze_with_ai(market_data):
    ...
    prompt = f"Analyze these Bursa Malaysia stocks. Suggest the TOP 12 most interesting SHARIAH setups. Output JSON ONLY. Data: {json.dumps(market_data)}"
...
def main():
    data = get_top_movers()
    if not data: return
    print(f"Discovered {len(data)} movers from the 120-stock universe.")
    ...
    print("Universe analysis complete. Dashboard synchronized.")
